...
public class CheckOut {
	...
	public String toString(){ 
		...
		for (DessertItem d : items){
			...
			} else if (d instanceof Sundaes){
				String c = "" + d.getCost();
				int sp = receiptWidth - ((Sundaes)d).getSundaeIceCreamType().length() - c.length() - 1;				
				everyItem = everyItem + ((Sundaes)d).getSundaeType() + "\n" + ((Sundaes)d).getSundaeIceCreamType() + insertSpc(sp) + DessertShoppe.cents2dollarsAndCents(d.getCost()) + "\n"; 
